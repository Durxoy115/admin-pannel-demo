...
const SalaryCreditSummary = () => {
  ...
  const fetchExpenseAmountHistory = async () => {
    try {
      const response = await fetch(`${url}/expense/salary-credit-summary/?salary_credit_id=yearly`, {
        headers: {
  ...
  const handleMonthlyCreditandDebit = (year) => {
    navigate(`/monthly-salary-credit-expense-amount/${year}`);
  };
  ...
  const handlePDFPreview = async () => {
    try {
      ...
      const heading = ["Year", "Start Date", "End Date", "Total Add Amount", "Total Salary Expense"];
      const value = ["year", "start_date", "end_date", "total_amount", "total_expense_salary"];
  ...
  return (
    <div className="bg-white mt-16 md:mt-20 px-1 md:px-8 ">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-black rounded-t-lg text-white pl-8 sm:pl-4 pr-3 sm:pr-4 py-2 sm:py-2">
        <h1 className="text-lg sm:text-lg mb-2 sm:mb-0">Yearly Total Salary Debits and Credits History</h1>
...
export default SalaryCreditSummary;