    ...
    try {
      ...
      if (response.data.success) {
        ...
        setFormData((prevData) => ({
          ...prevData,
          basic_per: data.basic || "",
          h_rent_per: data.house_rent || "",
          medical_allowance_per: data.medical_allow || "",
          mobile_allowance_per: data.mobile_allow || "",
          transport_per: data.transport_allow || "",
          others_per: data.others || "",
        }));
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setGlobalError("Error adding salary: " + (errorData.message || "Unknown error"));
      }
    } catch (error) {
      setGlobalError("Error adding salary: " + error.message);
    }
  };

  return (
    <div className="container mx-auto p-4 sm:p-6 bg-white rounded-lg shadow-md mt-16 max-w-full">
      <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-6">Add Employee Salary</h2>
