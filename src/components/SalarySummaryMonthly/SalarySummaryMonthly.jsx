...
const SalarySummaryMonthly = () => {
  ...
  const calculateTotals = (expenses) => {
    return expenses.reduce(
      (totals, expense) => ({
        totalReceive:
          totals.totalReceive + (parseFloat(expense.total_amount) || 0),
        totalExpense:
          totals.totalExpense + (parseFloat(expense.total_expense_salary) || 0),
      }),
  ...
  const fetchExpenseAmountHistory = async (year) => {
    try {
      const response = await fetch(
        `${url}/expense/salary-credit-summary/?salary_credit_id=monthly-${year}`,
        {
          headers: {
  ...
  const handleDailyCredit = (month, year) => {
    navigate(`/daily-salary-credit-list/${month}/${year}`);
  };
  const handleDailyExpense = (month, year) => {
    navigate(`/daily-salary-expense-list/${month}/${year}`);
  };
  ...
  const handlePDFPreview = async () => {
    try {
      ...
      const value = [
        ...
        "end_date",
        "total_amount",
        "total_expense_salary",
      ];
      const useCurrency = ["total_amount", "total_expense"];

      const pdfData =
        selectedExpenseId.length > 0
          ? filteredExpenses.filter((expense, index) =>
              selectedExpenseId.includes(index)
            )
          : filteredExpenses;
      ...
      const showTotalAmount = pdfData.reduce(
        (totals, expense) => ({
          Total_Receive:
            totals.Total_Receive + (parseFloat(expense.total_amount) || 0),
          Total_Expense:
            totals.Total_Expense +
            (parseFloat(expense.total_expense_salary) || 0),
        }),
  ...
  const years = [
    ...new Set(
      expenses.map((e) => new Date(e.start_date).getFullYear().toString())
    ),
  ]
    .sort()
    .reverse();
    ...
    <div className="bg-white mt-16 p-2 sm:p-6 w-full mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-black rounded-t-lg text-white px-4 py-2">
        <h1 className="text-sm sm:text-xl">
          Monthly Salary Credit and Debit History - {year}
        </h1>
            ...
            {filteredExpenses.length === 0 ? (
              ...
              filteredExpenses.map((expense, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="border-b p-2 text-sm">
                    {expense.currency_sign}{" "}
                    {(parseFloat(expense.total_expense_salary) || 0).toFixed(2)}
                  </td>
                  <td className="border-b p-2 text-sm">
                    <div className="flex justify-center gap-2 relative z-10">
                      <div className="relative group">
                        ...
                        />
                        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 text-xs bg-black text-white px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition z-50">
                          Credit
                        </span>
                      </div>
                      ...
                      <div className="relative group">
                        <BsListTask
                          className="bg-red-300 text-xl p-1 rounded-md cursor-pointer"
                          onClick={() =>
                            handleDailyExpense(
                              expense.month ||
                                new Date(expense.start_date).getMonth() + 1,
          ...
          {filteredExpenses.length > 0 && (
            <tfoot className="bg-gray-50">
              <tr>
                <td
                  colSpan="4"
                  className="p-2 text-right font-semibold text-sm"
                >
                  Total:
                </td>
...
export default SalarySummaryMonthly;
