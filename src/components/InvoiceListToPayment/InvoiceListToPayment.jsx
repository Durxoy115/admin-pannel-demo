...
const InvoiceListToPayment = () => {
  ...
  // Fetch invoice details based on invoice_id
      ...
      if (response.ok) {
        ...
        if (invoiceData && Object.keys(invoiceData).length > 0) {
          ...
          const totalPreviousPaid = previousPaymentList.reduce(
            (results, payment) => results + parseFloat(payment.amount || 0),
            0
          );
  ...
  // Automatically fetch invoice details when invoice_id is available
  useEffect(() => {
    if (invoice_id) {
