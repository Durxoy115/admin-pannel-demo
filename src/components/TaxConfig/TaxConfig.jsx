...
//   const canDeleteContact = permissions.includes("company.add_supportcontact");
  ...
  const fetchAddress = async () => {
    try {
      const response = await fetch(`${url}/company/tax-config/`, {
        headers: {
  ...
  const handleDeleteContact = async () => {
    ...
    try {
      const response = await fetch(
        `${url}/company/tax-config/?tax_config_id=${selectedContactId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Token ${token}`,
  ...
  const handleTaxConfig = () => {
    navigate("/add-tax-config");
  };

  const handleEdit = (id) => {
    navigate(`/edit-tax-config/${id}`);
  };
