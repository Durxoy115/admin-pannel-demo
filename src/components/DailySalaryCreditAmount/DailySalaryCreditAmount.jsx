...
const DailySalaryCreditAmount = () => {
  ...
  const fetchExpenseAmountHistory = async (month, year) => {
    try {
      const response = await fetch(
        `${url}/expense/salary-credit-summary/?salary_credit_id=daily-credit-${month}-${year}`,
        {
          headers: {
  ...
  const handlePDFPreview = async () => {
    try {
      const title = `Daily Credits History - ${year}-${month}`;
  ...
  const handleDeleteExpense = async (expenseId) => {
    try {
      const response = await fetch(
        `${url}/expense/salary-credit/?salary_credit_id=${expenseId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Token ${token}`,
    ...
    <div className="bg-white mt-16 p-4 sm:p-6 md:p-8 w-full mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-black rounded-t-lg text-white px-4 sm:px-6 py-2 sm:py-2">
        <h1 className="text-lg sm:text-xl mb-2 sm:mb-0">
          Daily Salary Credit History - {month} {year}
        </h1>
...
export default DailySalaryCreditAmount;