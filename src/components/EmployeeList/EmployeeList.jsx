...
const EmployeeList = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        setError("Error fetching employees: " + (data.message || "No data returned"));
      }
    } catch (error) {
  ...
  return (
    <div className="bg-white mt-16 p-4 sm:p-6 md:p-8 w-full mx-auto">
      ...
      <div className="p-3 rounded-md flex flex-col sm:flex-row gap-2 sm:gap-3 flex-wrap">
  ...
  <table className="min-w-full border border-gray-300 text-xs">
    <thead className="bg-gray-100 sticky top-0 z-10">
      <tr>
        ...
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Profile</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Full Name</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Employee ID</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Job Title</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Joining Date</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Contact</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Email</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Status</th>
        <th className="border-b border-gray-300 p-1 sm:p-2 text-left">Actions</th>
      </tr>
    </thead>
    <tbody>
      {filteredEmployees.length === 0 ? (
        <tr>
          <td colSpan="10" className="text-center p-3 text-gray-500 text-xs">
            No employees found
          </td>
        </tr>
      ) : (
        filteredEmployees.map((employee) => (
          <tr key={employee.id} className="hover:bg-gray-50">
            ...
            <td className="border-b border-gray-300 p-1 sm:p-2">
              <img
                src={employee.photo ? `${url}${employee.photo}` : "https://via.placeholder.com/40"}
                alt="Profile"
                className="w-8 h-8 rounded-full object-cover"
                onError={(e) => (e.target.src = "https://via.placeholder.com/40")}
              />
            </td>
            <td className="border-b border-gray-300 p-1 sm:p-2">{employee.full_name}</td>
            <td className="border-b border-gray-300 p-1 sm:p-2">{employee.employee_id}</td>
            <td className="border-b border-gray-300 p-1 sm:p-2">{employee.job_title}</td>
            <td className="border-b border-gray-300 p-1 sm:p-2">
            ...
            <td className="border-b border-gray-300 p-1 sm:p-2">{employee.contact}</td>
            <td className="border-b border-gray-300 p-1 sm:p-2">{employee.email}</td>
            <td className="border-b border-gray-300 p-1 sm:p-2">{employee.status}</td>
            <td className="border-b border-gray-300 p-1 sm:p-2">
