...
const EditSalaryConfig = () => {
  const { id } = useParams(); // Extract salary_config_id from URL
  ...
  useEffect(() => {
    const fetchSalaryConfig = async () => {
      try {
        const response = await axios.get(`${url}/company/salary-config/?salary_config_id=${id}`, {
          headers: {
        ...
        if (response.data.success) {
          ...
          setFormData({
            ...
            medical_allow: data.medical_allow,
            others: data.others,
            transport_allow: data.transport_allow,
            mobile_allow: data.mobile_allow,
            active: data.active,
          });
        } else {
          setGlobalError("Failed to fetch salary configuration: " + (response.data.message || "Unknown error"));
        }
      } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (formData.basic && (parseFloat(formData.basic) < 0 || parseFloat(formData.basic) > 100))
      errors.basic = "Basic percentage must be between 0 and 100";
    if (formData.house_rent && (parseFloat(formData.house_rent) < 0 || parseFloat(formData.house_rent) > 100))
      errors.house_rent = "House rent percentage must be between 0 and 100";
    if (formData.medical_allow && (parseFloat(formData.medical_allow) < 0 || parseFloat(formData.medical_allow) > 100))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/company/salary-config/?salary_config_id=${id}`, formDataToSend, {
        headers: {
          Authorization: `Token ${token}`,
      ...
      if (response.data.success) {
        ...
        setGlobalError("Failed to update salary configuration: " + (response.data.message || "Unknown error"));
      }
    } catch (error) {
      setGlobalError("Error updating salary configuration: " + (error.response?.data?.message || error.message));
    }
  };

  if (loading) {
    return (
      <div className="p-1 sm:p-6 lg:p-8 min-h-screen bg-gray-100 flex justify-center items-center">
